...
string huffman::decToBin(int inNum) {
    ...
    while (inNum > 0) {
        temp += (inNum % 2 + '0');
        //(inNum % 2 + '0') converts the remainder (0 or 1) to its ASCII character representation ('0' or '1') and appends it to the temp string.
        inNum /= 2;
    }
    res.append(8 - temp.length(), '0');
    //This line ensures that the binary representation is 8 bits long by adding leading zeros if necessary. It calculates how many zeros are needed (8 - length of temp) and appends them to the res string.
...
void huffman::buildTree(char a_code, string& path) {
    // The purpose of this function is to build a binary tree used in Huffman coding to represent characters and their associated Huffman codes. Here's a breakdown of the code:
    Node* curr = root;
    //This is a loop that iterates through each character in the path string, which represents the Huffman code for a character.
    for (int i = 0; i < path.length(); i++) {
        if (path[i] == '0') {
            //Inside the loop, it checks if the current character in the path is '0'. If it is, it means that the Huffman code instructs us to move to the left child of the current node.
            if (curr->left == NULL) {
                curr->left = new Node();
            }
            curr = curr->left;
    ...
    //Finally, the character a_code is assigned to the data field of the node pointed to by curr, indicating that this node represents the character associated with the Huffman code.
    curr->data = a_code;
}
...
void huffman::createTree() {
    ...
    //firts mistake here when size()!=0 done.
    while (tempPQ.size() != 1)
    {
        left = tempPQ.top();
        tempPQ.pop();
